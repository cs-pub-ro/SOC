...
module task12(
    ...
    // TODO: Modify address to repair hazards caused by pipelining or See the modifcations
	always @(negedge i_w_clk) begin
		case (i_w_address)
			...
			8'd9:	l_r_data = {1'b1, SUB_OPERATION,	5'd7, 5'd2}; //WaW 4
            //MOD R[7] R[2]      R[7] <- 0x0001
			8'd14:	l_r_data = {1'b1, MOD_OPERATION,	5'd7, 5'd2}; //WaW 9
            //DIV R[20] R[2]     R[20] <- 0x000A
			8'd15:	l_r_data = {1'b1, DIV_OPERATION,	5'd20, 5'd2};
            //MUL R[4] R[2]      R[4] <- 0x000A
			8'd16:	l_r_data = {1'b1, MUL_OPERATION,	5'd4, 5'd2};
            //DIV R[6] R[4]      R[6] <- 0x1999
			8'd21:	l_r_data = {1'b1, DIV_OPERATION,	5'd6, 5'd4}; //RaW 16
            //ADD R[0] R[7]      R[0] <- 0x0002
			8'd22:	l_r_data = {1'b1, ADD_OPERATION,	5'd0, 5'd7};
            //ADD R[4] R[6]      R[4] <- 0x19A3
			8'd26:	l_r_data = {1'b1, ADD_OPERATION,	5'd4, 5'd6}; //RaW 21
            //ADD R[2] R[20]      R[2] <- 0x000C
			8'd27:	l_r_data = {1'b1, ADD_OPERATION,	5'd2, 5'd20};
            //ADD R[0] R[4]      R[0] <- 0x19A5
			8'd31:	l_r_data = {1'b1, ADD_OPERATION,	5'd0, 5'd4}; //RaW 26
            //ADD R[0] R[2]      R[0] <- 0x19B1
			8'd36:	l_r_data = {1'b1, ADD_OPERATION,	5'd0, 5'd2}; //WaW 31 RaW 27
